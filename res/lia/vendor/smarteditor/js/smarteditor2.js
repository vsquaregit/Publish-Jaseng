...
nhn.HuskyRange = jindo.$Class({
	...
	*/
	...
	pasteHTML : function(sHTML, bBlock){
		...
		// 때문에 현재 위치의 컨테이너가 P태그이고 콘텐츠 내용이 block 요소인 경우 P태그를 쪼개고 그 사이에 콘텐츠를 DIV로 감싸서 넣도록 처리한다.
		...
		if(oStart.oLineBreaker && oStart.oLineBreaker.nodeName === "P" && (bBlock || clone.rxHasBlock.test(sHTML))){
			...
			// 콘텐츠 삽입 후에 북마크를 지운다.
			// 콘텐츠 삽입 전에 지우면 콘텐츠 삽입시 oNextSibling 가 북마크로 잡히는 경우 에러가 발생할 수 있음
			clone.removeStringBookmark(sBM);

			// 콘텐츠 삽입 후 윗라인 P태그에 아무런 내용이 없으면 제거한다.
...
//	iMinUndoInterval : 50,
	...
	_getStyleSize : function(oCurrentStyle){
		 ...
		 * 콘텐츠가 화면에서 사라진 것처럼 보이는 현상을 일으킨다.
		 * 
	...
	$ON_PASTE_HTML : function(sHTML, oPSelection, htOption){
		...
		if(!oNavigator.ie){
			...
			// 원인 확인 결과 콘텐츠 삽입 후 기존 Bookmark 삭제 시 갱신된 Selection 이 제대로 반영되지 않는 점이 있었습니다.
...
//}
/**
 * @pluginDesc WYSIWYG 영역에 붙여넣어지는 외부 콘텐츠를 정제하는 플러그인
 */
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_QuickEditor_Common, {
	...
	setOpenType : function(sType,bBol){
		// [SMARTEDITORSUS-1213] 작성된 콘텐츠 수정 화면에서 사진이 로드되자마자 바로 사진을 클릭하면 QuickEditor를 띄우는 데 문제가 있음
